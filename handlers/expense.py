...
async def expense_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    intent_data = await asyncio.to_thread(ai_helper.parse_expense_intent, message_text)
...
async def handle_record_expense(update, context, user_id, intent_data):
    ...
    try:
        ...
        daily_summary = await asyncio.to_thread(db_ops.get_daily_summary, user_id)
        ...
        await update.message.reply_text("記錄失敗，請稍後再試。")
...
async def handle_query_expenses(update, context, user_id, intent_data):
    ...
    try:
        ...
        if time_range == 'today':
            summary = await asyncio.to_thread(db_ops.get_daily_summary, user_id)
            response = ai_helper.format_expense_summary(summary)

        elif time_range == 'month':
            summary = await asyncio.to_thread(db_ops.get_monthly_summary, user_id)
            response = ai_helper.format_expense_summary(summary)
            ...
            expenses = await asyncio.to_thread(
                db_ops.get_user_expenses, user_id,
                start_of_week, None, 'expense'
            )
            incomes = await asyncio.to_thread(
                db_ops.get_user_expenses, user_id,
                start_of_week, None, 'income'
            )
...
async def set_budget_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if not context.args:
            await update.message.reply_text("請提供預算金額，例如: /setbudget 30000")
            return

        budget = float(context.args[0])

        if budget <= 0 or budget > 100_000_000:
            await update.message.reply_text("請輸入合理的預算金額 (1 ~ 100,000,000)")
            return
        ...
        await update.message.reply_text(f"✅ 已設定月度預算: ${budget:,.0f}")
